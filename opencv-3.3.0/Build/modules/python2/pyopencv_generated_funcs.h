...
static PyObject* pyopencv_cv_GaussianBlur(PyObject* , PyObject* args, PyObject* kw)
{
    ...
    const char* keywords[] = { "src", "ksize", "sigmaX", "dst", "sigmaY", "borderType", "q1", "q2", NULL };
    if( PyArg_ParseTupleAndKeywords(args, kw, "OOd|Odidd:GaussianBlur", (char**)keywords, &pyobj_src, &pyobj_ksize, &sigmaX, &pyobj_dst, &sigmaY, &borderType, &q1, &q2) &&
        pyopencv_to(pyobj_src, src, ArgInfo("src", 0)) &&
        pyopencv_to(pyobj_dst, dst, ArgInfo("dst", 1)) &&
        pyopencv_to(pyobj_ksize, ksize, ArgInfo("ksize", 0)) )
    {
        ERRWRAP2(cv::GaussianBlur(src, dst, ksize, sigmaX, sigmaY, borderType, q1, q2));
...
static PyObject* pyopencv_cv_getGaussianKernel(PyObject* , PyObject* args, PyObject* kw)
{
    ...
    const char* keywords[] = { "ksize", "sigma", "q", "ktype", NULL };
    if( PyArg_ParseTupleAndKeywords(args, kw, "idd|i:getGaussianKernel", (char**)keywords, &ksize, &sigma, &q, &ktype) )
    {
        ERRWRAP2(retval = cv::getGaussianKernel(ksize, sigma, q, ktype));
        return pyopencv_from(retval);
    }
